...
5. **Purpose:** Set to `prefetch` when making [prefetch](/v2/data-props/prefetching)requests.
6. **X-Inertia-Partial-Component:** The component name for [partial reloads](/v2/data-props/partial-reloads).
...
11. **X-Inertia-Error-Bag:** Specifies which error bag to use for [validation errors](/v2/the-basics/validation).
12. **X-Inertia-Infinite-Scroll-Merge-Intent:** Indicates whether the requested data should be appended or prepended when using [Infinite scroll](/v2/data-props/infinite-scroll).
...
7. **mergeProps:** Array of prop keys that should be merged (appended) during navigation. See the [merging props](/v2/data-props/merging-props) documentation for details.
...
12. **deferredProps:** Configuration for client-side lazy loading of props. See the [deferred props](/v2/data-props/deferred-props) documentation for details.
